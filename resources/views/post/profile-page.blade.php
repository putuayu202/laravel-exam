<x-app-layout>
    ...
    <div class="p-6 sm:p-12 dark:bg-gray-50 dark:text-gray-800">
        <div class="flex flex-col space-y-4 md:space-y-0 md:space-x-6 md:flex-row">
            <img src="{{ Storage::url($data['data_user']->userDetail->profil_pic) }}" alt="" class="self-center flex-shrink-0 w-24 h-24 border rounded-full md:justify-self-start dark:bg-gray-500 dark:border-gray-300">
            <div class="flex flex-col">
                <h4 class="text-lg font-semibold text-center md:text-left">{{ $data['data_user']->userDetail->username }}</h4>
                <p class="dark:text-gray-600">{{ $data['data_user']->userDetail->bio }}</p>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 grid grid-cols-3">
            @foreach($data['data_post'] as $post)
                <x-card-image :post="$post" />
            @endforeach
        </div>
    </div>
